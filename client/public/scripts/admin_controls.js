...
if(addFeatureBtn){
    addFeatureBtn.addEventListener('click', (e)=>{
        ...
        if(newFeature.length > 1){
            featureList.innerHTML += `<div class="input-group mb-3 input-group-block"><div class="input-group-prepend"><span class="input-group-text text-light bg-dark" id="basic-addon1"><i class="fas fa-trash delete-feature-icon text-danger"></i></span></div><li class="list-group-item admin-feature-item"></i><input class="form-control feature-input" type="text" value="${newFeature.trim()}"></li></div>`
            featureInput.value = '';
            FeaturesToInput();
        };
    });
